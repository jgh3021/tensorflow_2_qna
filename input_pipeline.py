...
def decode_record(record, name_to_features):
  ...
  for name in list(example.keys()):
    t = example[name]
    if name != 'unique_id' and t.dtype == tf.int64:
...
def file_based_input_fn_builder(input_files, name_to_features):
...
def create_tf_squad_dataset(file_paths, seq_length, batch_size, is_training=True):
  """Creates input dataset from (tf)records files for train/eval."""
  name_to_features = {
  ...
  input_fn = file_based_input_fn_builder(file_paths, name_to_features)
  dataset = input_fn()

  def _select_data_from_record(record):
    x, y = {}, {}
    for name, tensor in record.items():
      if name in ('start_positions', 'end_positions','answer_types'):
        y[name] = tensor
      else:
        x[name] = tensor
    return (x, y)
