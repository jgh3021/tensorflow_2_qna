...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
  ...
  for (example_index, example) in enumerate(examples):
    ...
    parag_text_split = [ t_.lower().strip() for t_ in paragraph_text.split() ]
    ...
    for i in range(len(para_tokens)):
      ...
      str_tok = paragraph_text[start_orig_pos:end_orig_pos+1]
      if ' ' in str_tok:
        if i != 0:
          parag_idx += 1
          if len(parag_to_token_index) == 0:
            parag_to_token_index.append((0 , len(token_to_parag_index)-1))
          else:
            parag_to_token_index.append((parag_to_token_index[-1][1]+1 , len(token_to_parag_index)-1 ))
    ...
    assert len(token_to_parag_index) == len(tok_start_to_orig_index)
    ...
    for (doc_span_index, doc_span) in enumerate(doc_spans):
      ...
      cur_tok_token_to_parag_index = []

      tokens.append(tokenizer.sp_model.PieceToId("[CLS]"))
      segment_ids.append(0)
      p_mask.append(0)
      cur_tok_token_to_parag_index.append(-1)
      ...
      for i in range(doc_span.length):
        ...
        p_mask.append(0)
        cur_tok_token_to_parag_index.append(token_to_parag_index[split_token_index])
      tokens.append(tokenizer.sp_model.PieceToId("[SEP]"))
      segment_ids.append(1)
      p_mask.append(1)
      cur_tok_token_to_parag_index.append(-1)

      assert len(cur_tok_token_to_parag_index) == len(segment_ids)
      ...
      if example_index < 40:
        ...
        logging.info("example.question_text: %s" % (example.question_text))
        logging.info("paragraph_text: %s" % (paragraph_text))
        logging.info("tok_start_to_orig_index: %s" % (tok_start_to_orig_index))
        logging.info("tok_end_to_orig_index: %s" % (tok_end_to_orig_index))
        logging.info("tok_cat_text: %s" % (tok_cat_text))
        logging.info("para_tokens: %s" % (para_tokens))
        logging.info("chartok_to_tok_index: %s" % (chartok_to_tok_index))
        logging.info("token_to_parag_index: %s" % (token_to_parag_index))
        logging.info("parag_to_token_index: %s" % (parag_to_token_index))

        logging.info("start_position: %s" % (start_position-doc_offset))
        logging.info("end_position: %s" % (end_position-doc_offset))
        logging.info("example.start_position: %s" % (example.start_position))        
        ...
        logging.info("cur_tok_token_to_parag_index: %s" % (cur_tok_token_to_parag_index)) 
